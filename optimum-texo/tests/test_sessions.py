...
class SettingsTests(unittest.TestCase):
    ...
    def test_toml_validation(self):
        """Load valid values and reject invalid types, ranges, and unknown keys."""
...
class SessionTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_idle_consumers_release_completed_and_canceled_crops(self):
    ...
    async def test_parallel_sessions_and_queue_limit(self):
        """Two native owners overlap while active and pending crops remain bounded."""
    ...
    async def test_cancellation_budgets_and_recovery(self):
        """Canceled crops are skipped, token budgets stay separate, and failures release owners."""
        calls = []

        def generate(model, tokenizer, images, max_length):
            """Fail one budget and return identifiable results for unaffected callers."""
            calls.append((len(images), max_length))
            if max_length == 2:
                raise RuntimeError("simulated inference failure")
            return [{"text": str(max_length), "output_tokens": 1} for _ in images]
