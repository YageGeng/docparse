...
def main() -> None:
    ...
    source_hash = hashlib.sha256(
        (source / "encoder_model.onnx").read_bytes()
    ).hexdigest()
    if (
        source_hash
    ...
    session = ort.InferenceSession(
        str(source / "encoder_model.onnx"), providers=["CUDAExecutionProvider"]
    )
    ...
    # ORT can also return sparse tensors or containers; this encoder must return a dense array.
    if not isinstance(actual, np.ndarray):
        raise TypeError("Texo encoder output must be a dense NumPy array")
    ...
    for dimension, size in zip(
        model.graph.input[0].type.tensor_type.shape.dim[1:], [3, 384, 384]
    ):
    ...
    report = {
        ...
        "deployment_sha256": hashlib.sha256(
            (target / "encoder_model.onnx").read_bytes()
        ).hexdigest(),
    }
