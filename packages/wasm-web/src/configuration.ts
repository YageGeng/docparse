...
export function formulaEngineError(value: unknown, enabled = true): string | undefined {
  ...
  // Every formula consumer uses the backend selected by the browser Worker.
  if (engine.type !== "mineru" && engine.session_size !== undefined) {
    const count = engine.session_size;
    if (typeof count !== "number" || !Number.isSafeInteger(count) || count < 1) return "session_size must be a positive safe integer";
  }
