...
export interface FormulaResult { engine: string; id: string; label: "inline_formula" | "display_formula"; bbox: Bbox; block_id: string | null; line_id: string | null; text_item_range: { start: number; end: number } | null; text_spans?: TableTextSpan[]; table_cell: [number, number] | null; latex: string | null; markdown: string | null; error: string | null }
...
/** Plus-L uses a pinned BPE tokenizer instead of a model YAML configuration. */
