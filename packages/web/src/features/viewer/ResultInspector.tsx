...
export function ResultInspector({
  ...
  const [openedMarkdown, setOpenedMarkdown] = useState<string>();
  const [copied, setCopied] = useState(false);
  const [copyError, setCopyError] = useState(false);
  // Remember only a preview actually opened in this document, including navigation while Markdown is selected.
  useEffect(() => { if (tab === "markdown") setOpenedMarkdown(documentId); }, [documentId, tab]);
  ...
  return (
    <section id="result-pane" className="inspector" aria-label="解析结果">
      <div className="inspector-heading">
        <div>
          <h2>解析结果</h2>
          <p>
            {tab === "markdown" ? "完整文档 · Markdown" : page
      ...
      <Tabs value={tab} onValueChange={setTab} className="inspector-tabs-root">
        {/* Keep a single table renderer in the content view; the tab bar only switches representations. */}
        <TabsList variant="line" className="inspector-tabs" aria-label="结果视图">
          <TabsTrigger value="text">
            <FileText size={14} />
            内容
          </TabsTrigger>
          <TabsTrigger value="markdown"><BookOpen size={14} />Markdown</TabsTrigger>
            ...
            <>
              ...
              {page.blocks.map((block) => (
                <article
                  id={`content-${block.id}`}
                  key={block.id}
                  className={`content-block ${block.id === selected ? "is-selected" : ""} ${["algorithm", "content", "chart", "table"].includes(typeof block.label === "string" ? block.label : "") ? "is-spatial" : ""}`}
                >
                  <button
      ...
      <div className="inspector-footer">
        ...
        <span>{tab === "markdown" ? "Markdown 展示完整文档，不随 PDF 页码切换" : "点击 PDF 区域或内容标签，可联动定位"}</span>
      </div>
    </section>
  );
}
