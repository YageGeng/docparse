...
export function initRuntime(features, dist) {
	if (globalThis.ort && globalThis.ort[INIT_SYMBOL]) {
    ...
    // Module Workers load the ESM distribution without constructing a fake DOM.
    if (typeof document === 'undefined') {
        const moduleName = dist.scriptName.replace(/\.js$/, '.mjs');
        return import(relative(moduleName)).then(runtime => {
            const namespace = runtime.default ?? runtime;
            if (!namespace.InferenceSession || !namespace.Tensor) {
                throw new Error('ORT module does not export InferenceSession and Tensor');
            }
            globalThis.ort = { ...namespace };
            Object.defineProperty(globalThis.ort, INIT_SYMBOL, { value: true });
            return true;
        });
    }
	...
	return new Promise((resolve, reject) => {
		...
		script.addEventListener('load', () => {
			if (!('ort' in globalThis)) {
				return reject(new Error('script loaded but ort not defined'));
			}

			Object.defineProperty(globalThis.ort, INIT_SYMBOL, {
				value: true,
				configurable: false,
				enumerable: false,
				writable: false
