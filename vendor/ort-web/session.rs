    ...
    pub async fn from_url(
        ...
        Ok(Session {
            ...
            js: binding::InferenceSession::create_from_uri(uri, &options.js)
                .await
                .map_err(|e| {
                    Error::new(OrtErrorCode::ORT_FAIL, value_to_string(&e))
                })?,
...
impl Drop for Session {
    /// Releases the JavaScript session after the final Rust owner has finished using it.
    fn drop(&mut self) {
        let session = self
            .js
            .clone()
            .unchecked_into::<binding::InferenceSession>();
