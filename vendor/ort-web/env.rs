...
impl Environment {
    ...
    pub unsafe fn cast_from_sys<'e>(
        ptr: *const ort_sys::OrtEnv,
    ) -> &'e Environment {
        unsafe { &*ptr.cast::<Environment>() }
    }

    pub unsafe fn cast_from_sys_mut<'e>(
        ptr: *mut ort_sys::OrtEnv,
    ) -> &'e mut Environment {
    ...
    pub fn send_telemetry_event(&self, event: TelemetryEvent) {
        ...
        let _ = match event {
            TelemetryEvent::SessionInit => {
                #[allow(unused_unsafe)]
                unsafe {
                    binding::track_session_init(env!("CARGO_PKG_VERSION"))
                }
            }
        };
    }
