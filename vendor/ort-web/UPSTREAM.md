...
- `binding/session.rs` and `api.rs`: support per-output `preferredOutputLocation` through `ort_web.preferred_output_location.<output>` session config entries. Unconfigured sessions retain the upstream CPU-output default. Texo requests GPU-resident hidden states/KV caches and CPU logits; its browser regression asserts actual tensor locations.
