...
use crate::{
    ...
    tensor::{
        SyncDirection, Tensor, TensorData, TypeInfo, create_buffer,
        onnx_to_dtype,
    },
    util::value_to_string,
};

unsafe extern "system" fn CreateEnv(
...
unsafe extern "system" fn EnableTelemetryEvents(
    env: *const OrtEnv,
) -> OrtStatusPtr {
...
unsafe extern "system" fn DisableTelemetryEvents(
    env: *const OrtEnv,
) -> OrtStatusPtr {
    let env = unsafe { Environment::cast_from_sys_mut(env.cast_mut()) };
...
unsafe extern "system" fn Run(
    ...
    Error::new_sys(
        ...
        "Synchronous `Session::run` is not supported in ort-web; use `run_async()`.",
    )
}
...
unsafe fn RunAsync(
    ...
    let fut = Box::pin(async move {
        ...
        // Forward fetches to JS so unused model outputs are never returned across the boundary.
        match session
            .js
            .run_with_fetches(
                inputs.into_iter(),
                output_names.iter().map(|&name| (name, None)),
            )
            .await
        {
            Ok(outputs) => {
                let output_names: Vec<String> =
                    output_names.iter().map(|&name| name.to_string()).collect();
                let output_view = unsafe {
                    core::slice::from_raw_parts_mut(
                        output_ptrs.as_mut_ptr().cast::<*mut Tensor>(),
                        output_ptrs.len(),
                    )
                };
                ...
                for (name, mut tensor) in outputs {
                    if let Some(index) = output_names
                        ...
                        .find_map(|(o_name, output)| {
                            if name == *o_name { Some(output) } else { None }
                        })
                    {
                        if !session.disable_sync {
                            if let Err(e) =
                                tensor.sync(SyncDirection::Rust).await
                            {
                                return Error::new_sys(
                                    OrtErrorCode::ORT_FAIL,
                                    format!(
                                        "Failed to synchronize output '{name}': {e}"
                                    ),
                                );
                            }
                        }
            ...
            Err(e) => Error::new_sys(
                OrtErrorCode::ORT_FAIL,
                format!("Failed to run session: {}", value_to_string(&e)),
            ),
        }
    }) as Pin<Box<dyn Future<Output = OrtStatusPtr>>>;
    unsafe { core::mem::transmute(fut) }
...
unsafe extern "system" fn CreateSessionOptions(
    options: *mut *mut OrtSessionOptions,
) -> OrtStatusPtr {
    unsafe {
        options.write(
            (Box::leak(Box::new(SessionOptions::new())) as *mut SessionOptions)
                .cast(),
        )
    };
    OrtStatusPtr::default()
...
unsafe extern "system" fn SessionOptionsAppendExecutionProvider(
    ...
    let execution_providers =
        options.js.execution_providers.get_or_insert_default();

    let Ok(options) =
        unsafe { core::slice::from_raw_parts(provider_options_keys, num_keys) }
            .iter()
            .zip(
                unsafe {
            ...
            .map(|(k, v)| {
                Ok((
                    unsafe { CStr::from_ptr(*k) }.to_str()?,
                    unsafe { CStr::from_ptr(*v) }.to_str()?,
                ))
            })
        ...
        return Error::new_sys(
            OrtErrorCode::ORT_FAIL,
            "EP options contains invalid UTF-8",
        );
    };
    ...
    match provider_name.to_string_lossy().as_ref() {
        ...
        "WebGPU" => {
            execution_providers.push(binding::ExecutionProvider::WebGPU {
                preferred_layout: match options
                    .get("ep.webgpuexecutionprovider.preferredLayout")
                {
        ...
        "WebNN" => {
            execution_providers.push(binding::ExecutionProvider::WebNN {
                power_preference: match options.get("powerPreference") {
                    Some(&"default") => {
                        Some(binding::WebNNPowerPreference::Default)
                    }
                    Some(&"high-performance") => {
                        Some(binding::WebNNPowerPreference::HighPerformance)
                    }
                    Some(&"low-power") => {
                        Some(binding::WebNNPowerPreference::LowPower)
                    }
                    _ => None,
                },
        ...
        x => {
            return Error::new_sys(
                ...
                format!("Provider '{x}' not supported"),
            );
        }
    }
...
unsafe extern "system" fn CloneSessionOptions(
    ...
    unsafe {
        out_options.write(
            (Box::leak(Box::new(options.clone())) as *mut SessionOptions)
                .cast(),
        )
    };
    OrtStatusPtr::default()
...
unsafe extern "system" fn SessionGetInputCount(
...
unsafe extern "system" fn SessionGetOutputCount(
    session: *const OrtSession,
    out: *mut usize,
) -> OrtStatusPtr {
    let session = unsafe { &*session.cast::<Session>() };
...
unsafe extern "system" fn SessionGetOverridableInitializerCount(
    _session: *const OrtSession,
    out: *mut usize,
) -> OrtStatusPtr {
    unsafe { out.write(0) };
    OrtStatusPtr::default()
...
unsafe extern "system" fn SessionGetInputTypeInfo(
    session: *const OrtSession,
    index: usize,
    ...
    if !metadata.is_tensor {
        return Error::new_sys(
            OrtErrorCode::ORT_FAIL,
            "non-tensor types are not currently supported",
        );
    }

    unsafe {
        type_info.write(TypeInfo::new_sys_from_value_metadata(&metadata))
    };
    OrtStatusPtr::default()
}

unsafe extern "system" fn SessionGetOutputTypeInfo(
    session: *const OrtSession,
    index: usize,
    type_info: *mut *mut OrtTypeInfo,
) -> OrtStatusPtr {
...
unsafe extern "system" fn CreateRunOptions(
    out: *mut *mut OrtRunOptions,
) -> OrtStatusPtr {
    unsafe {
        out.write(
            (Box::leak(Box::new(RunOptions::new())) as *mut RunOptions).cast(),
        )
    };
    OrtStatusPtr::default()
}
...
unsafe extern "system" fn CreateTensorAsOrtValue(
    ...
    match binding::Tensor::new_from_buffer(
        dtype,
        create_buffer(dtype, &shape),
        &shape,
    ) {
        Ok(tensor) => {
            unsafe {
                out.write(
                    (Box::leak(Box::new(Tensor::from_tensor(tensor)))
                        as *mut Tensor)
                        .cast(),
                )
            };
        ...
        Err(e) => Error::new_sys(
            OrtErrorCode::ORT_FAIL,
            format!("Failed to create tensor: {}", value_to_string(&e)),
        ),
    }
}
...
unsafe extern "system" fn CreateTensorWithDataAsOrtValue(
    ...
    match unsafe { Tensor::from_ptr(dtype, p_data, p_data_len, &shape) } {
        Ok(tensor) => {
            unsafe {
                out.write((Box::leak(Box::new(tensor)) as *mut Tensor).cast())
            };
            OrtStatusPtr::default()
        }
        Err(e) => Error::new_sys(
...
unsafe extern "system" fn IsTensor(
...
unsafe extern "system" fn GetTensorMutableData(
    value: *mut OrtValue,
    out: *mut *mut ffi::c_void,
) -> OrtStatusPtr {
    ...
    match &mut tensor.data {
        ...
        TensorData::External { buffer } => {
            if let Some(buffer) = buffer {
                ...
                Error::new_sys(
                    OrtErrorCode::ORT_FAIL,
                    "External data is not synchronized; you should call `TensorExt::sync`.",
                )
            }
        }
    }
...
unsafe extern "system" fn CastTypeInfoToTensorInfo(
...
unsafe extern "system" fn GetOnnxTypeFromTypeInfo(
    _type_info: *const OrtTypeInfo,
    out: *mut ONNXType,
) -> OrtStatusPtr {
...
unsafe extern "system" fn CreateTensorTypeAndShapeInfo(
    out: *mut *mut OrtTensorTypeAndShapeInfo,
) -> OrtStatusPtr {
    unsafe {
        out.write(
            TypeInfo::new_sys(binding::DataType::Float32, Vec::new()).cast(),
        )
    };
    OrtStatusPtr::default()
}

unsafe extern "system" fn SetTensorElementType(
...
unsafe extern "system" fn SetDimensions(
    info: *mut OrtTensorTypeAndShapeInfo,
    ...
    let info = unsafe { &mut *info.cast::<TypeInfo>() };
    info.shape =
        unsafe { core::slice::from_raw_parts(dim_values.cast(), dim_count) }
            .to_vec();
    OrtStatusPtr::default()
}

unsafe extern "system" fn GetTensorElementType(
...
unsafe extern "system" fn GetDimensionsCount(
    info: *const OrtTensorTypeAndShapeInfo,
    out: *mut usize,
) -> OrtStatusPtr {
    let info = unsafe { &*info.cast::<TypeInfo>() };
...
unsafe extern "system" fn GetDimensions(
    info: *const OrtTensorTypeAndShapeInfo,
    dim_values: *mut i64,
...
unsafe extern "system" fn GetTensorShapeElementCount(
    info: *const OrtTensorTypeAndShapeInfo,
    out: *mut usize,
) -> OrtStatusPtr {
    let info = unsafe { &*info.cast::<TypeInfo>() };
...
unsafe extern "system" fn GetTensorTypeAndShape(
...
unsafe extern "system" fn CreateMemoryInfo(
    ...
    match MemoryInfo::from_location(&*device_name.to_string_lossy()) {
        Some(inf) => {
            unsafe {
                *out = (Box::leak(Box::new(inf)) as *mut MemoryInfo).cast()
            };
            OrtStatusPtr::default()
        }
        None => Error::new_sys(
...
unsafe extern "system" fn CreateCpuMemoryInfo(
    _type: OrtAllocatorType,
    _mem_type: OrtMemType,
    out: *mut *mut OrtMemoryInfo,
) -> OrtStatusPtr {
    unsafe {
        *out = (Box::leak(Box::new(MemoryInfo {
            location: binding::DataLocation::Cpu,
        })) as *mut MemoryInfo)
            .cast()
    };
    OrtStatusPtr::default()
...
unsafe extern "system" fn CompareMemoryInfo(
...
unsafe extern "system" fn MemoryInfoGetName(
    ptr: *const OrtMemoryInfo,
    out: *mut *const ffi::c_char,
) -> OrtStatusPtr {
    let info = unsafe { &*ptr.cast::<MemoryInfo>() };
    unsafe {
        out.write(info.location_exposed().unwrap_or(c"").as_ptr().cast())
    };
    OrtStatusPtr::default()
}

unsafe extern "system" fn MemoryInfoGetId(
    _ptr: *const OrtMemoryInfo,
    out: *mut ffi::c_int,
) -> OrtStatusPtr {
    unsafe { out.write(0) };
    OrtStatusPtr::default()
...
unsafe extern "system" fn MemoryInfoGetMemType(
    _ptr: *const OrtMemoryInfo,
    out: *mut OrtMemType,
) -> OrtStatusPtr {
...
unsafe extern "system" fn MemoryInfoGetType(
    _ptr: *const OrtMemoryInfo,
...
unsafe extern "system" fn GetAllocatorWithDefaultOptions(
    out: *mut *mut OrtAllocator,
) -> OrtStatusPtr {
    unsafe {
        out.write(
            (&crate::memory::DEFAULT_CPU_ALLOCATOR as *const Allocator)
                .cast_mut()
                .cast(),
        )
    };
...
unsafe extern "system" fn ReleaseTensorTypeAndShapeInfo(
    input: *mut OrtTensorTypeAndShapeInfo,
) {
...
unsafe extern "system" fn CreateAllocator(
    _session: *const OrtSession,
    ...
    if mem_info.location != binding::DataLocation::Cpu {
        return Error::new_sys(
            ...
            "Only CPU allocators are supported.",
        );
    }

    unsafe {
        out.write(
            (Box::leak(Box::new(Allocator::new())) as *mut Allocator).cast(),
        )
    };
    OrtStatusPtr::default()
}
...
unsafe extern "system" fn GetTensorMemoryInfo(
    value: *const OrtValue,
    mem_info: *mut *const OrtMemoryInfo,
) -> OrtStatusPtr {
    let tensor = unsafe { &*value.cast::<Tensor>() };
    unsafe {
        mem_info.write((&tensor.memory_info as *const MemoryInfo).cast())
    };
    OrtStatusPtr::default()
}

unsafe extern "system" fn MemoryInfoGetDeviceType(
    ptr: *const OrtMemoryInfo,
    out: *mut OrtMemoryInfoDeviceType,
) {
    let memory_info = unsafe { &*ptr.cast::<MemoryInfo>() };
    unsafe {
        out.write(match memory_info.location {
            binding::DataLocation::Cpu | binding::DataLocation::CpuPinned => {
...
unsafe extern "system" fn GetBuildInfoString() -> *const ffi::c_char {
    concat!(
        "ORT Build Info: backend=ort-web, version=",
