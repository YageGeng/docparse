...
#[utoipa::path(
    ...
    description = "Submit exactly one multipart file field containing a PDF. Idempotency-Key is also the job UUID: reuse it after a lost response. Identical content returns the existing job; different content returns 4091001. Input is persisted before HTTP 202. Upload size and timeout are deployment settings (defaults: 512 MiB and 300 seconds).",
    ...
    responses(
        ...
        (status = 409, description = "Idempotency key refers to different PDF content (4091001)", body = ApiErrorResponse),
...
pub async fn upload(
    ...
    let task = async {
        ...
        state
            ...
            .await?;
        let job = Jobs::submit(&state.db, id, &hash).await.with_context(
