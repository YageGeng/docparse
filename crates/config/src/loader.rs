...
impl ConfigLoader {
    ...
    pub fn load_raw(self) -> Result<RawConfig, ConfigError> {
        ...
        let config_path = requested_path.canonicalize().map_err(|source| {
    ...
    fn environment_values(
        ...
        if let Some(provider) = &self.env_provider {
            let mut values: Dict = provider.extract().map_err(|source| {
            ...
            let profile = match values.remove("profile") {
                Some(value) => {
                    Some(value.deserialize::<String>().map_err(|source| {
                        ConfigError::Environment {
                            source: Box::new(source),
                        }
                    })?)
