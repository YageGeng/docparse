...
impl Block {
    ...
    fn render_markdown_text(
        ...
        for line in &self.lines {
            let rendered = line.render_markdown_formulas(placeholder, formulas);
...
impl crate::Line {
    ...
    pub(crate) fn render_markdown_formulas(
        ...
        for formula in formulas
            .iter()
            .filter(|formula| formula.line_id.as_ref() == Some(&self.id))
        {
            ...
            if !ranges.is_empty() {
                replacements.push((ranges, markdown));
            }
        }
        if replacements.is_empty() {
            return render_line(self, placeholder);
        }
        ...
        crate::render::replace_formula_ranges(&source, replacements, false)
    }
}
