...
impl<'a> TableAssembler<'a> {
    ...
    pub(crate) fn reconstruct_external(
        ...
        geometry_policy: super::TsrGeometryPolicy,
        ...
        let grid =
            CellGrid::try_from((request, block.bbox, input, geometry_policy))?;
        ...
        let grid = match geometry_policy {
            super::TsrGeometryPolicy::Predicted => {
                geometry.assign_predicted(grid, &self.evidence.rules)
            }
            super::TsrGeometryPolicy::Declared => geometry
                .assign_words(grid.table())
                .ok_or_else(|| {
                    "declared cells do not uniquely own every source word"
                        .to_owned()
                })
                .and_then(|assignment| {
                    grid.bind_words(assignment, spans.len())
                }),
        }
        .map_err(|message| TableStructureError::SourceText { message })?;
        let table = self
...
// Keep private-type tests beside the crate's other tests without exposing assembly internals.
#[path = "../../tests/table_assembly"]
#[cfg(test)]
mod tests {
    use super::*;
    mod fixtures;
