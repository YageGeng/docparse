//! Shared warm-model measurements for local and server-owned PDFium providers.
use docparse_config::{
...
pub async fn run(
    provider: Arc<dyn docparse_core::PdfiumProvider>,
    ...
    let process_override = args
        .next()
        .map(|value| value.to_string_lossy().parse::<usize>())
        .transpose()?;
    if args.next().is_some()
        || process_override.is_some_and(|n| Some(n) != processes)
    {
        return Err("unexpected or inconsistent PDFium process override".into());
    }
    ...
    record(
        &mut output,
        json!({"kind":"configuration", "config":config_json, "documents":paths.len(), "concurrency":concurrency, "repeats":repeats, "pdfium_processes":processes,
    ...
    let mut builder = DocParser::builder()
        .pdfium_provider(Arc::clone(&provider))
    ...
    if let Some(count) = processes {
        // Holding every lease while warming ensures each process is exercised, not just the first idle slot.
        let started = Instant::now();
        let mut sessions = Vec::new();
        for _ in 0..count {
            let session = provider
                .open(
                    docparse_core::PdfInput::Path(workspace.join(
                        "crates/core/tests/fixtures/pdf/table_layout.pdf",
                    )),
                    config.runtime(),
                    Timings::default(),
                )
            ...
            for page_number in 1..=session.page_count() {
                session.pre_scan_page(page_number, None).await?;
                session.render_page(page_number, config.render()).await?;
            }
        ...
        record(
            &mut output,
            json!({"kind":"pdfium_warmed", "processes":count, "seconds":started.elapsed().as_secs_f64()}),
        )?;
    }
    let image =
    ...
    for round in 1..=repeats {
        ...
        record(
            &mut output,
            json!({"kind":"summary", "round":round, "concurrency":concurrency, "pdfium_processes":processes,
