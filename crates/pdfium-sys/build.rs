...
fn main() {
    ...
    if target_arch == "wasm32" {
        ...
        // the final cdylib's build script (crates/docparse-wasm/build.rs),
...
fn resolve_pdfium_dirs() -> (PathBuf, PathBuf) {
    ...
    if let (Ok(lib), Ok(inc)) =
        (env::var("PDFIUM_LIB_PATH"), env::var("PDFIUM_INCLUDE_PATH"))
    {
...
fn dirs_cache() -> PathBuf {
    ...
    if target_os == "windows" {
        ...
        let home =
            env::var("USERPROFILE").expect("USERPROFILE env var not set");
...
fn run_bindgen(include_dir: &Path) {
    ...
    #[cfg(not(feature = "bindgen"))]
    {
        let _ = include_dir;
        let pregenerated =
            Path::new(env!("CARGO_MANIFEST_DIR")).join("bindings.rs");
