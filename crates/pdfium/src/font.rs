...
impl Font<'_> {
    ...
    pub fn font_type(&self) -> FontType {
        ...
        match t {
            ...
            pdfium_sys::FPDF_FONT_TYPE_FPDF_FONTTYPE_TRUETYPE => {
                FontType::TrueType
            }
            pdfium_sys::FPDF_FONT_TYPE_FPDF_FONTTYPE_TYPE0 => FontType::Type0,
            pdfium_sys::FPDF_FONT_TYPE_FPDF_FONTTYPE_TYPE3 => FontType::Type3,
            pdfium_sys::FPDF_FONT_TYPE_FPDF_FONTTYPE_CID_TYPE0 => {
                FontType::CidType0
            }
            pdfium_sys::FPDF_FONT_TYPE_FPDF_FONTTYPE_CID_TYPE2 => {
                FontType::CidType2
            }
            _ => FontType::Unknown,
        }
    ...
    pub fn ascent(&self, font_size: f32) -> Option<f32> {
        let mut val: f32 = 0.0;
        let ok = unsafe {
            ffi!(FPDFFont_GetAscent(self.handle, font_size, &mut val))
        };
    ...
    pub fn descent(&self, font_size: f32) -> Option<f32> {
        let mut val: f32 = 0.0;
        let ok = unsafe {
            ffi!(FPDFFont_GetDescent(self.handle, font_size, &mut val))
        };
        if ok != 0 { Some(val) } else { None }
    }

    /// Get glyph width using the raw character code.
    pub fn glyph_width_from_char_code(
        &self,
        char_code: u32,
        font_size: f32,
    ) -> Option<f32> {
    ...
    pub fn glyph_path_segments(
        ...
        let count =
            unsafe { ffi!(FPDFGlyphPath_CountGlyphSegments(glyph_path)) };
        let mut segments = Vec::new();
        for i in 0..count {
            let segment = unsafe {
                ffi!(FPDFGlyphPath_GetGlyphPathSegment(glyph_path, i))
            };
            if segment.is_null() {
                break;
            }
            ...
            let mut y: f32 = 0.0;
            let ok = unsafe {
                ffi!(FPDFPathSegment_GetPoint(segment, &mut x, &mut y))
            };
            if ok == 0 {
                x = 0.0;
                y = 0.0;
    ...
    pub fn encoding(&self) -> Option<String> {
        let len = unsafe {
            ffi!(FPDFFont_GetEncoding(self.handle, std::ptr::null_mut(), 0))
        } as usize;
        if len == 0 {
            return None;
        }
    ...
    pub fn char_glyph_index(&self, char_code: u32) -> Option<u32> {
        let idx =
            unsafe { ffi!(FPDFFont_GetCharGlyphIndex(self.handle, char_code)) };
