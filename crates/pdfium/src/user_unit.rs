...
//! source is the docparse PDFium build's `FPDFPage_GetUserUnit` export, which
...
pub(crate) fn scan_user_units(data: &[u8]) -> Vec<UserUnitEntry> {
    ...
    while let Some(rel) = find(&data[from..], NEEDLE) {
        ...
        if !user_unit.is_finite()
            || user_unit <= MIN_USER_UNIT
            || user_unit > MAX_USER_UNIT
        {
            continue;
        }
...
pub(crate) fn match_user_unit(
    entries: &[UserUnitEntry],
    width: f32,
    height: f32,
) -> f32 {
    for entry in entries {
        // FPDF_GetPageSizeByIndexF reflects /Rotate, so compare both ways.
        if (close(width, entry.media_width)
            && close(height, entry.media_height))
            || (close(width, entry.media_height)
                && close(height, entry.media_width))
        {
...
fn parse_media_box(object: &[u8]) -> Option<(f32, f32)> {
    ...
    for value in &mut values {
        *value = parse_number_at(object, i)?;
        ...
        while i < object.len()
            && matches!(object[i], b'+' | b'-' | b'.' | b'0'..=b'9')
        {
            i += 1;
        }
    }
