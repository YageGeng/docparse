...
//!
//! For docparse's purposes we want a flat list of nodes with:
//! - the role string
...
impl Page<'_, '_> {
    ...
    pub fn structure_tree(&self) -> Vec<StructureElement> {
        ...
        for index in 0..count {
            let element =
                unsafe { ffi!(FPDF_StructTree_GetChildAtIndex(tree, index)) };
            if !element.is_null() {
    ...
    pub fn struct_tree(&self, view_box: &RectF) -> Vec<StructNode> {
        ...
        for i in 0..count {
            let elem =
                unsafe { ffi!(FPDF_StructTree_GetChildAtIndex(tree, i)) };
            if !elem.is_null() {
...
fn read_structure_element(
    ...
    for index in 0..count {
        let child =
            unsafe { ffi!(FPDF_StructElement_GetChildAtIndex(elem, index)) };
        if child.is_null() {
            let object_number =
                unsafe { ffi!(FPDF_StructElement_GetChildObjNum(elem, index)) };
            if object_number >= 0 {
...
fn read_marked_content_ids(elem: pdfium_sys::FPDF_STRUCTELEMENT) -> Vec<i32> {
    let count =
        unsafe { ffi!(FPDF_StructElement_GetMarkedContentIdCount(elem)) };
    let mut ids = Vec::with_capacity(count.max(0) as usize);
    for index in 0..count {
        let id = unsafe {
            ffi!(FPDF_StructElement_GetMarkedContentIdAtIndex(elem, index))
        };
        if id >= 0 {
            ids.push(id);
        }
...
fn read_attributes(
    ...
    let attr_count =
        unsafe { ffi!(FPDF_StructElement_GetAttributeCount(elem)) };
    for attr_index in 0..attr_count {
        let attr = unsafe {
            ffi!(FPDF_StructElement_GetAttributeAtIndex(elem, attr_index))
        };
        if attr.is_null() {
            continue;
        }
        let value_count =
            unsafe { ffi!(FPDF_StructElement_Attr_GetCount(attr)) };
        for value_index in 0..value_count {
            ...
            let value_type =
                unsafe { ffi!(FPDF_StructElement_Attr_GetType(value)) };
            let parsed = if value_type == pdfium_sys::FPDF_OBJECT_BOOLEAN as i32
            {
                let mut boolean = 0;
                (unsafe {
                    ffi!(FPDF_StructElement_Attr_GetBooleanValue(
                        value,
                        &mut boolean
                    ))
                } != 0)
                ...
                let mut number = 0.0;
                (unsafe {
                    ffi!(FPDF_StructElement_Attr_GetNumberValue(
                        value,
                        &mut number
                    ))
                } != 0)
            ...
            {
                read_attribute_string(value)
                    .map(StructureAttributeValue::String)
            } else {
                None
            };
...
fn read_attribute_name(
    attr: pdfium_sys::FPDF_STRUCTELEMENT_ATTR,
    index: i32,
) -> Option<String> {
    let mut needed = 0;
    unsafe {
...
fn read_attribute_string(
    value: pdfium_sys::FPDF_STRUCTELEMENT_ATTR_VALUE,
) -> Option<String> {
    let mut needed = 0;
    unsafe {
...
fn collect_mcid_bboxes(
    ...
    let mut map: std::collections::HashMap<i32, RectF> =
...
fn walk_element(
    ...
    for i in 0..n_mcids {
        let m = unsafe {
            ffi!(FPDF_StructElement_GetMarkedContentIdAtIndex(elem, i))
        };
        if m >= 0 {
            mcids.push(m);
        }
    ...
    for i in 0..n_children {
        ...
        let child_mcid = unsafe {
            ffi!(FPDF_StructElement_GetChildMarkedContentID(elem, i))
        };
    ...
    for i in 0..n_children {
        let child_elem =
            unsafe { ffi!(FPDF_StructElement_GetChildAtIndex(elem, i)) };
...
fn read_element_type(elem: pdfium_sys::FPDF_STRUCTELEMENT) -> String {
    read_widestring(|buf, len| unsafe {
        ffi!(FPDF_StructElement_GetType(elem, buf, len))
    })
}

fn read_alt_text(elem: pdfium_sys::FPDF_STRUCTELEMENT) -> Option<String> {
    let s = read_widestring(|buf, len| unsafe {
        ffi!(FPDF_StructElement_GetAltText(elem, buf, len))
    });
