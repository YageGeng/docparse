...
/// must remain alive until the handle is closed, even though docparse leaves
...
impl<'lib> Document<'lib> {
    ...
    pub fn flatten_form_widgets(
        &self,
        index: i32,
    ) -> Result<Option<Page<'_, 'lib>>, PdfiumError> {
        {
    ...
    pub fn file_version(&self) -> Option<i32> {
        let mut version = 0;
        let ok =
            unsafe { ffi!(FPDF_GetFileVersion(self.handle, &mut version)) };
    ...
    fn walk_bookmark(
        ...
        while !cur.is_null() {
            ...
            let child =
                unsafe { ffi!(FPDFBookmark_GetFirstChild(self.handle, cur)) };
            if !child.is_null() {
                self.walk_bookmark(child, level.saturating_add(1), out);
            }

            cur =
                unsafe { ffi!(FPDFBookmark_GetNextSibling(self.handle, cur)) };
        }
    }
}
...
impl FormEnvironment<'_, '_> {
    ...
    /// docparse's document initialization sequence.
...
fn read_bookmark_title(bookmark: pdfium_sys::FPDF_BOOKMARK) -> String {
    let needed = unsafe {
        ffi!(FPDFBookmark_GetTitle(bookmark, std::ptr::null_mut(), 0))
    } as usize;
    if needed < 2 {
        return String::new();
    }
