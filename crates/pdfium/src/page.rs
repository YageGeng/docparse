...
impl<'doc, 'lib: 'doc> Page<'doc, 'lib> {
    ...
    pub fn viewport_size(&self, view_box: &RectF) -> (f32, f32) {
        ...
        let mut height =
            (view_box.top - view_box.bottom).abs() * self.user_unit;
    ...
    pub fn view_box(&self) -> Option<RectF> {
        ...
        let ok =
            unsafe { ffi!(FPDF_GetPageBoundingBox(self.handle, &mut rect)) };
        if ok != 0 {
            Some(RectF {
                left: rect.left,
                top: rect.top,
    ...
    pub fn bounds_to_viewport(
        &self,
        view_box: &RectF,
        page_bounds: &RectF,
    ) -> RectF {
        let (ll_x, ll_y) = self.page_to_viewport(
            view_box,
            page_bounds.left,
            page_bounds.bottom,
        );
        let (ur_x, ur_y) =
            self.page_to_viewport(view_box, page_bounds.right, page_bounds.top);
    ...
    /// wrapped around the render, mirroring docparse's extraction pipeline.
    ...
    fn render_bitmap(
        ...
        if let Some(form) = form {
            // NOTE: like docparse's extraction pipeline, popup annotations are
    ...
    /// `None` when the page has no content objects. Mirrors docparse's
    /// `Parse_getContentBounds` extraction behavior.
    ...
    pub fn image_objects(
        ...
        for i in 0..obj_count {
            ...
            if w > page_width * max_page_coverage
                && h > page_height * max_page_coverage
            {
                continue;
            }
    ...
    pub fn render_image_object(
        &self,
        image_obj_index: usize,
    ) -> Result<Bitmap<'lib>, PdfiumError> {
        let obj_count = unsafe { ffi!(FPDFPage_CountObjects(self.handle)) };
    ...
    pub fn links(&self, view_box: &RectF) -> Vec<PdfLink> {
        ...
        loop {
            ...
            let quad_count =
                unsafe { ffi!(FPDFLink_CountQuadPoints(link_annot)) };
            let mut emitted = false;
            for q in 0..quad_count {
                let mut quad = pdfium_sys::FS_QUADPOINTSF::default();
                let ok = unsafe {
                    ffi!(FPDFLink_GetQuadPoints(link_annot, q, &mut quad))
                };
                if ok == 0 {
                    continue;
                }
            ...
            let got =
                unsafe { ffi!(FPDFLink_GetAnnotRect(link_annot, &mut rect)) };
            if got == 0 {
                continue;
            }
            let page_bounds = RectF {
    ...
    pub fn annotations(&self, view_box: &RectF) -> Vec<PdfAnnotation> {
        ...
        for index in 0..count {
            ...
            if unsafe { ffi!(FPDFAnnot_HasAttachmentPoints(annot)) } != 0 {
                let quad_count =
                    unsafe { ffi!(FPDFAnnot_CountAttachmentPoints(annot)) };
                quadpoint_rects.reserve(quad_count);
                for quad_index in 0..quad_count {
                    let mut quad = pdfium_sys::FS_QUADPOINTSF::default();
                    if unsafe {
                        ffi!(FPDFAnnot_GetAttachmentPoints(
                            annot, quad_index, &mut quad
                        ))
                    } == 0
                    {
                        continue;
                    ...
                    quadpoint_rects
                        .push(self.bounds_to_viewport(view_box, &bounds));
                }
            }
    ...
    pub fn form_widget_text_rects(&self, view_box: &RectF) -> Vec<RectF> {
        ...
        for index in 0..count {
            ...
            if unsafe { ffi!(FPDFAnnot_GetSubtype(annot)) }
    ...
    pub fn text_objects_overlap(
        ...
        for i in 0..count {
            let obj = unsafe { ffi!(FPDFPage_GetObject(self.handle, i)) };
            if obj.is_null()
                || unsafe { ffi!(FPDFPageObj_GetType(obj)) }
    ...
    pub fn flatten_form_widgets_for_display(&self) -> bool {
        ...
        for index in 0..count {
            ...
            if subtype != pdfium_sys::FPDF_ANNOT_WIDGET as i32 {
                ...
                if flags & pdfium_sys::FPDF_ANNOT_FLAG_HIDDEN as i32 == 0 {
                    let hidden_flags =
                        flags | pdfium_sys::FPDF_ANNOT_FLAG_HIDDEN as i32;
                    let changed =
                        unsafe { (api.set_flags)(annot, hidden_flags) } != 0;
                    unsafe { ffi!(FPDFPage_CloseAnnot(annot)) };
                    if !changed {
                        restore_annotation_flags(
                            &api,
                            self.handle,
                            &suppressed,
                        );
        ...
        let result = unsafe {
            (api.flatten)(self.handle, pdfium_sys::FLAT_NORMALDISPLAY as i32)
        };
    ...
    pub fn form_fields(
        ...
        for annotation_index in 0..count {
            let annot = unsafe {
                ffi!(FPDFPage_GetAnnot(self.handle, annotation_index))
            };
            if annot.is_null() {
                continue;
            }
            ...
            let parent = unsafe {
                ffi!(FPDFAnnot_GetLinkedAnnot(annot, c"Parent".as_ptr()))
            };
            let name = first_present([
                read_form_string(
                    ...
                    |handle, annotation, buffer, len| unsafe {
                        ffi!(FPDFAnnot_GetFormFieldName(
            ...
            let value = first_present([
                read_form_string(
                    ...
                    annot,
                    |handle, annotation, buffer, len| unsafe {
                        ffi!(FPDFAnnot_GetFormFieldValue(
                            handle, annotation, buffer, len
                        ))
                    },
                ),
            ...
            let raw_type =
                unsafe { ffi!(FPDFAnnot_GetFormFieldType(form.handle, annot)) };
            let object_number =
                match unsafe { ffi!(FPDFAnnot_GetObjNum(annot)) } {
                    number if number > 0 => Some(number),
                    _ => None,
                };
            let id = name
                ...
                .unwrap_or_else(|| {
                    format!("page-{page_number}-widget-{widget_index}")
                });
            let rect = annotation_rect(self, annot, view_box);
            let option_count =
                unsafe { ffi!(FPDFAnnot_GetOptionCount(form.handle, annot)) };
            let mut options = Vec::new();
            let mut selected_options = Vec::new();
            for option_index in 0..option_count.max(0) {
                if let Some(label) =
                    read_form_option_label(form.handle, annot, option_index)
                {
                    if unsafe {
                        ffi!(FPDFAnnot_IsOptionSelected(
                            form.handle,
                            annot,
                            option_index
                        ))
            ...
            let is_checkable = raw_type
                == pdfium_sys::FPDF_FORMFIELD_CHECKBOX as i32
            ...
            let control_count = is_checkable
                .then(|| unsafe {
                    ffi!(FPDFAnnot_GetFormControlCount(form.handle, annot))
                })
                .filter(|value| *value >= 0);
            let control_index = is_checkable
                .then(|| unsafe {
                    ffi!(FPDFAnnot_GetFormControlIndex(form.handle, annot))
                })
                .filter(|value| *value >= 0);
            ...
            out.push(PdfFormField {
                ...
                export_value,
                field_flags: unsafe {
                    ffi!(FPDFAnnot_GetFormFieldFlags(form.handle, annot))
                },
                control_count,
                control_index,
                checked,
...
fn annotation_paints_text_shallow(annot: pdfium_sys::FPDF_ANNOTATION) -> bool {
    ...
    (0..object_count).any(|object_index| {
        ...
        !object.is_null()
            && unsafe { ffi!(FPDFPageObj_GetType(object)) }
                == pdfium_sys::FPDF_PAGEOBJ_TEXT as i32
    })
}
...
fn first_present<const N: usize>(
...
fn read_form_string(
    ...
    let end = units.saturating_sub(usize::from(
        buffer.get(units.saturating_sub(1)) == Some(&0),
    ));
...
fn annotation_subtype_name(
    subtype: pdfium_sys::FPDF_ANNOTATION_SUBTYPE,
) -> &'static str {
    const NAMES: [&str; 29] = [
        "unknown",
        "text",
...
fn read_uri_path(
    ...
    let needed = unsafe {
        ffi!(FPDFAction_GetURIPath(doc, action, std::ptr::null_mut(), 0))
    } as usize;
    if needed < 2 {
        return None;
    }
...
fn collect_path_objects(
    ...
    if obj_type == pdfium_sys::FPDF_PAGEOBJ_FORM as i32 {
        ...
        for i in 0..n {
            let child = unsafe {
                ffi!(FPDFFormObj_GetObject(obj, i as std::os::raw::c_ulong))
            };
            if child.is_null() {
                continue;
            }
    ...
    let dm_ok = unsafe {
        ffi!(FPDFPath_GetDrawMode(obj, &mut fill_mode, &mut stroke_bool))
    };
    ...
    let fill_color = read_color(|r, g, b, a| unsafe {
        ffi!(FPDFPageObj_GetFillColor(obj, r, g, b, a))
    });
    ...
    for si in 0..n_segs {
        ...
        let mut sy = 0.0f32;
        let pt_ok =
            unsafe { ffi!(FPDFPathSegment_GetPoint(seg, &mut sx, &mut sy)) };
        if pt_ok == 0 {
            continue;
        }
...
fn collect_filled_paths(
    ...
    let ok =
        unsafe { ffi!(FPDFPath_GetDrawMode(obj, &mut fill_mode, &mut stroke)) };
...
impl ViewportTransform {
    ...
    pub fn transform_bounds(&self, page_bounds: &RectF) -> RectF {
        let (ll_x, ll_y) =
            self.transform_point(page_bounds.left, page_bounds.bottom);
        let (ur_x, ur_y) =
            self.transform_point(page_bounds.right, page_bounds.top);
        RectF {
            left: ll_x.min(ur_x),
            top: ll_y.min(ur_y),
            right: ll_x.max(ur_x),
