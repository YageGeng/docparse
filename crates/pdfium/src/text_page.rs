...
impl TextChar<'_> {
    ...
    pub fn font_weight(&self) -> i32 {
        unsafe {
            ffi!(FPDFText_GetFontWeight(self.text_page.handle, self.index))
        }
    }
    ...
    pub fn angle(&self) -> f32 {
        unsafe {
            ffi!(FPDFText_GetCharAngle(self.text_page.handle, self.index))
        }
    }
    ...
    pub fn text_object(&self) -> Option<pdfium_sys::FPDF_PAGEOBJECT> {
        let obj = unsafe {
            ffi!(FPDFText_GetTextObject(self.text_page.handle, self.index))
        };
    ...
    pub fn is_generated(&self) -> bool {
        unsafe {
            ffi!(FPDFText_IsGenerated(self.text_page.handle, self.index)) == 1
        }
    }
